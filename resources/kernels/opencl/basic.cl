...
void traceRayThroughLens(__global LinearBVHNode* linearNodes, 
                         __global Primitive* primitives, 
  ...
  float4 transmissionDirection = refract(ray->direction, normal, 1.0, material->ior);

  RayPayload rayPayload2 = {0, 0, FLT_MAX, 0, 0};
  Ray ray2 = {position, transmissionDirection};
  ...
  barycentrics = (float3)(1.0 - rayPayload2.u - rayPayload2.v, rayPayload2.u, rayPayload2.v);
  positionA = (float3)(primitive->positionA[0], primitive->positionA[1], primitive->positionA[2]);
  positionB = (float3)(primitive->positionB[0], primitive->positionB[1], primitive->positionB[2]);
  positionC = (float3)(primitive->positionC[0], primitive->positionC[1], primitive->positionC[2]);
  position = (float4)(positionA * barycentrics.x + positionB * barycentrics.y + positionC * barycentrics.z, 1.0);
  normalA = (float3)(primitive->normalA[0], primitive->normalA[1], primitive->normalA[2]);
  normalB = (float3)(primitive->normalB[0], primitive->normalB[1], primitive->normalB[2]);
  normalC = (float3)(primitive->normalC[0], primitive->normalC[1], primitive->normalC[2]);
  normal = (float4)(normalA * barycentrics.x + normalB * barycentrics.y + normalC * barycentrics.z, 0.0);

  transmissionDirection = refract(transmissionDirection, -normal, material->ior, 1.0);
...
float3 shade(__global LinearBVHNode* linearNodes, 
  ...
  if (rayPayload.hitType == 1) {
    ...
    outputColor = (float3)(material->diffuse[0], material->diffuse[1], material->diffuse[2]);
  }
